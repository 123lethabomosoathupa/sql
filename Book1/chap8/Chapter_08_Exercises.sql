...
-- Chapter 8 covers summary functions (`count()`, `sum()`, `max()`,
--  `min()`) and `GROUP BY` to analyze data, using U.S.
--   library visits as the example. The key lesson:
--    group your data, watch for dirty values, and use
--     `HAVING` (not `WHERE`) to filter grouped results.
-- ----------------------------------------------------------------
-- Exercise 1:
